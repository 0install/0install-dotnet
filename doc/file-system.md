...
The <xref:ZeroInstall.Store.FileSystem.IBuilder> interface represents an implementation directory being constructed.  
<xref:ZeroInstall.Store.FileSystem.IForwardOnlyBuilder> is a more limited subset that only allows the addition of files, not the modification of files that have already been added.
