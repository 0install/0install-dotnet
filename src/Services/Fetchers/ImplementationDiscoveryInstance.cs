...
[PrimaryConstructor]
[Equatable]
internal sealed partial class ImplementationDiscoveryInstance
{
    private readonly ushort _port;
    private readonly IEnumerable<IPAddress> _potentialIPs;
    ...
    public async Task<Uri?> GetImplementationAsync(ManifestDigest manifestDigest, CancellationToken cancellationToken)
        => _confirmedIP == null
            ? await ResultRacer.For(_potentialIPs, (ip, innerCancellationToken) => TryGetUriAsync(ip, manifestDigest, innerCancellationToken), cancellationToken).GetResultAsync().ConfigureAwait(false)
    ...
    {
        var uri = new UriBuilder {Scheme = "http", Host = ip.ToString(), Port = _port, Path = $"{manifestDigest}.tar.gz"}.Uri;
