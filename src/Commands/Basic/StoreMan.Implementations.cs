...
namespace ZeroInstall.Commands.Basic
{
    partial class StoreMan
    {
        ...
        public class Add : StoreSubCommand
        {
            ...
            private void BuildImplementation(IImplementationBuilder builder)
            {
            ...
            public override ExitCode Execute()
            {
                ...
                try
                {
                    ...
                    var manifest = File.Exists(manifestPath)
                        ? Manifest.Load(manifestPath, ManifestFormat.FromPrefix(id))
                        : null;
                    store.Add(digest, builder => Handler.RunTask(new ReadDirectory(path, builder, manifest)));
                    return ExitCode.OK;
                }
                catch (ImplementationAlreadyInStoreException ex)
                {
        ...
        public class Export : StoreSubCommand
        {
            ...
            public override ExitCode Execute()
            {
                ...
                string manifestPath = Path.Combine(sourceDirectory, Manifest.ManifestFile);
                var manifest = File.Exists(manifestPath)
                    ? Manifest.Load(manifestPath, ManifestFormat.FromPrefix(AdditionalArgs[0]))
                    : null;

                using var builder = ArchiveBuilder.Create(outputArchive, mimeType);
                Handler.RunTask(new ReadDirectory(sourceDirectory, builder, manifest));
