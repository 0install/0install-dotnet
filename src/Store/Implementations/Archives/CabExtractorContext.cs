...
namespace ZeroInstall.Store.Implementations.Archives
{
    ...
    internal sealed record CabExtractorContext(IImplementationBuilder Builder, Stream Stream, Func<string, string> NormalizePath, CancellationToken CancellationToken)
        : IUnpackStreamContext
    {
        ...
        public Stream? OpenFileWriteStream(string path, long fileSize, DateTime lastWriteTime)
        {
            ...
            _thread = new Thread(() => Builder.AddFile(relativePath, stream, lastWriteTime)) {IsBackground = true};
            _thread.Start();
            return stream;
        }
