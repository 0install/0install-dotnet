...
namespace ZeroInstall.Store.Implementations
{
    ...
    public static class ImplementationStoreUtils
    {
        ...
        public static void Verify(this IImplementationStore store, ManifestDigest manifestDigest, ITaskHandler handler)
        {
            #region Sanity checks
            if (store == null) throw new ArgumentNullException(nameof(store));
            ...
            try
            {
                Verify(store.GetPath(manifestDigest) ?? throw new ImplementationNotFoundException(manifestDigest), manifestDigest, handler);
            }
            catch (DigestMismatchException ex) when (ex.ExpectedDigest != null)
            {
                ...
                if (handler.Ask(
                    ...
                    store.Remove(new ManifestDigest(ex.ExpectedDigest), handler);
            }
        }
