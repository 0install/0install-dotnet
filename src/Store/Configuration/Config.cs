...
/// </summary>
[Serializable]
[UnconditionalSuppressMessage("ReflectionAnalysis", "IL2026", Justification = "[DefaultValue] attributes are only used for property grid display, not by trim-sensitive code paths.")]
public sealed partial class Config : IEnumerable<KeyValuePair<string, string>>, ICloneable<Config>, IEquatable<Config>
{
    ...
    /// <summary>
    /// Use the SAT Solver by default instead of trying the Backtracking Solver first.
    /// </summary>
    [DefaultValue(false), Category("Solver"), DisplayName(@"Prefer SAT solver"), Description("Use the SAT Solver by default instead of trying the Backtracking Solver first.")]
    public bool PreferSatSolver { get; set; }
    ...
    public Config()
    {
        _metaData = new()
        {
            ...
            ["prefer_sat_solver"] = ConfigProperty.For(() => PreferSatSolver),
