...
#else
        public static readonly PosixBinary Reference32 = new PosixBinary {RelativePath = "elf32", Name = "elf32", Architecture = new Architecture(OS.Linux, Cpu.All)};

        [Fact]
        public void Elf() => TestAnalyze(Reference32, executable: true);
#endif

        [Fact]
        public void NotExecutable() => new PosixBinary().Analyze(baseDirectory: Directory, file: Deploy(Reference32, xbit: false))
            .Should().BeFalse();

        [Fact]
        public void NotElf() => new PosixBinary().Analyze(baseDirectory: Directory, file: Deploy(PosixScriptTest.Reference, xbit: true))
            .Should().BeFalse();
    }
}
