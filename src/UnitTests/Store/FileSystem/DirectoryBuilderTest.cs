...
public class DirectoryBuilderTest : IDisposable
{
    ...
    [Fact]
    public void CopyFromFileToMissingDestination()
    {
        using var sourceDir = Build([
            new TestFile("file") { Contents = Data, LastWrite = 1337 }
        ]);

        var metadata = new CopyFromStep { Source = "file" };
        _builder.Invoking(x => x.CopyFrom(metadata, sourceDir, new SilentTaskHandler()))
                .Should().Throw<IOException>("If source is a file, destination must be specified");
    }
