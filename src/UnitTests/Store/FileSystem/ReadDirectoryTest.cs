...
public class ReadDirectoryTest : IDisposable
{
    ...
    {
        Assert.SkipWhen(UnixUtils.IsUnix, "Manifest file is only used to detect executable bits and symlinks on non-Unix file systems");
    ...
    public void SubDir()
    {
        ...
        var mock = new Mock<IForwardOnlyBuilder>();
        new ReadDirectory(_tempDir, mock.Object, subDir: "subdir").Run(TestContext.Current.CancellationToken);
